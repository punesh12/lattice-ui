...
import { Badge, LatticeLogo } from '@lattice-ui/ui'
...
const DocsCommand = dynamic(() => import('./docs-command').then((m) => m.DocsCommand), { ssr: false })
  ...
  return (
    <header className="sticky top-0 z-sticky border-b border-border bg-background/80 backdrop-blur-md">
      <div
        ...
        style={{ display: 'flex', alignItems: 'center', height: 56, paddingLeft: 16, paddingRight: 16, gap: 12 }}
      >
      ...
      <div
        id="docs-mobile-nav"
        className={cn('marketing-mobile-nav max-h-[calc(100vh-3.5rem)] overflow-y-auto border-t border-border bg-background lattice-fade-in')}
